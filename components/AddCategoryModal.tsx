...
export const AddCategoryModal: React.FC<AddCategoryModalProps> = ({ isOpen, onClose, onSave, initialData, categories, title = 'Category', label = 'Name', duplicateMessage = 'This item already exists.' }) => {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h2 className="text-xl font-bold text-indigo-600">{initialData ? `Edit ${title}` : `Add ${title}`}</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-xl flex justify-end space-x-3">
            ...
            <button type="submit" className="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-sm">{initialData ? 'Save Changes' : `Add ${title}`}</button>
          </div>
        </form>
      </div>
    </div>
