...
export const AddUserModal: React.FC<AddUserModalProps> = ({ isOpen, onClose, onSave, users }) => { 
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-in fade-in zoom-in duration-200">
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input name="mobile" type="text" required placeholder="10 digits" value={formData.mobile} onChange={handleChange} className={`w-full px-4 py-2.5 bg-white border rounded-lg focus:ring-2 focus:ring-indigo-100 outline-none text-gray-900 placeholder-gray-400 ${errors.mobile ? 'border-red-500' : 'border-gray-200'}`} />
          ...
          <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-xl flex justify-end space-x-3">
            <button type="button" onClick={onClose} className="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors uppercase font-bold text-xs">Cancel</button>
            <button type="submit" className="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-md uppercase font-bold text-xs">Add User</button>
          </div>
        </form>
      </div>
    </div>
